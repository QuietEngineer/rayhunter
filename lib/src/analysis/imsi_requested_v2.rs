...
impl ImsiRequestedAnalyzer {
    pub fn new() -> Self {
        Self { packet_num: 0, state: State::Unattached, timeout_counter: 0, flag: None }
    }
    ...
    fn transition(&mut self, next_state: State) {
        match (&self.state, &next_state) {
            ...
            */
            ...
            (_, State::AuthAccept) |
            (State::AuthAccept, State::Disconnect) => {
                self.state = next_state;
            ...
            (_, State::IdentityRequest) if self.state != State::AttachRequest => {
                self.flag = Some((true, Severity::High, "Identity requested without Attach Request".to_string()));
                self.state = next_state;
            }

            // IMSI to Disconnect without AuthAccept
            (State::IdentityRequest, State::Disconnect) => {
                self.flag = Some((false, Severity::Low, "Disconnected after Identity Request without Auth Accept".to_string()));
                self.state = next_state;
            }
...
impl Analyzer for ImsiRequestedAnalyzer {
    ...
    fn get_description(&self) -> Cow<str> {
        Cow::from("Tests whether the ME sends an Identity Request NAS message without either an associated attach request or auth accept message")
    }
    ...
    fn analyze_information_element(&mut self, ie: &InformationElement) -> Option<Event> {
        ...
        if self.state == State::IdentityRequest {
            ...
            if self.timeout_counter > PACKET_THRESHHOLD {
                self.flag = Some((true, Severity::High, "Identity request happened without auth request followup".to_string()));
            }
        }
